...
# from modules import Rnn_net
from modules import rnn_attn_net
...
class ExamplePredictor(BasePredictor):
    ...
    def __init__(self, embedding,
        ...
        # self.model = Rnn_net(embedding.size(1))
        self.model = rnn_attn_net(embedding.size(1))
    ...
    def _run_iter(self, batch, training):
        with torch.no_grad():
            context = self.embedding(batch['context'].long().to(self.device))
            options = self.embedding(batch['options'].long().to(self.device))
