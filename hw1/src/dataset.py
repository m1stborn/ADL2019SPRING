...
class DialogDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if n_positive != 0:        # train
            positive_indices = [0]
        else:
            positive_indices = []   # test or valid

        # TODO: sample negative indices
        if self.n_negative != -1:
            negative_indices = random.sample(range(len(negatives)), n_negative)
        else:
    ...
    def collate_fn(self, datas):
        ...
        batch['context_lens'] = [len(data['context']) if len(data['context']) > 0
                                 else 1
                                for data in datas]
...
def pad_to_len(arr, padded_len, padding=0):
    ...
    if len(arr) < padded_len:
        return np.pad(arr,(0, padded_len - len(arr)), 'constant',constant_values=padding).tolist()
    else:
